...
nis2scan prüft Ihre Cloud-Umgebung (AWS, Azure, GCP) automatisiert gegen die 10 Kernmaßnahmen des §30 Abs. 2 BSIG (NIS2-Umsetzungsgesetz). Jedes Finding wird direkt auf den Gesetzestext und ISO 27001:2022 gemappt und ist damit auditfähig dokumentiert.
...
**Detaillierte Anleitung:** [docs/getting-started.md](https://github.com/letaible/nis2scan/blob/main/docs/getting-started.md): Installation, Cloud-Setup (AWS/Azure/GCP), Konfiguration, CLI-Referenz, Troubleshooting.
...
|----------|-------------|------|
| **BSIG (BSI-Gesetz)** | Kerngesetz mit allen NIS2-Pflichten für Unternehmen | [gesetze-im-internet.de](https://www.gesetze-im-internet.de/bsig_2025/) |
...
| **ISO/IEC 27001:2022** | Internationaler ISMS-Standard, an dem sich NIS2 orientiert | [ISO.org](https://www.iso.org/standard/27001) |
...
|--------|----------|-------------|-------------|-------------------|-----------|
| ✅ | AWS-NR1-001 | AWS Config Recorder aktiv (alle Regionen) | HIGH | §30(2) Nr.1: "Konzepte in Bezug auf die Risikoanalyse", denn Risikoanalyse setzt Sichtbarkeit aller Assets voraus | A.5.1, A.8.9 |
...
| ✅ | AWS-NR8-006 | ELB/ALB TLS Policy ≥ TLS 1.2 | HIGH | §30(2) Nr.8 + "Stand der Technik" (§30(2) S.1), denn TLS 1.0/1.1 ist nicht mehr Stand der Technik | A.8.24 |
...
| ✅ | AWS-NR5-003 | SSM Patch Manager Compliance | HIGH | §30(2) Nr.5: Wartung, das heißt Patches zeitnah einspielen, "Stand der Technik" (§30(2) S.1) | A.8.8, A.8.9 |
...
| ✅ | AWS-NR9-005 | IAM Policies ohne wildcard (*) Berechtigungen | HIGH | §30(2) Nr.9: Zugriffskontrolle nach Least-Privilege-Prinzip | A.5.15, A.8.3 |
| ✅ | AWS-NR9-006 | S3 Bucket Policies ohne Principal: * | CRITICAL | §30(2) Nr.9: Keine anonymen Zugriffe auf Datenbestände | A.5.15, A.8.3 |
...
| ✅ | AZ-NR9-003 | NSG Rules ohne offene Ports zu Internet | HIGH | §30(2) Nr.9: Netzwerk-Zugriffskontrolle | A.8.20, A.8.22 |
...
| Status | Check-ID | Beschreibung | Schweregrad | §30 Text-Referenz | ISO 27001 |
|--------|----------|-------------|-------------|-------------------|-----------|
| ✅ | AWS-NR10-001 | Root-Account MFA aktiviert | CRITICAL | §30(2) Nr.10: "Verwendung von Lösungen zur Multi-Faktor-Authentifizierung", denn Root hat die höchsten Privilegien | A.8.5 |
...
### Ebene 2: Was muss in den Richtlinien je §30-Bereich stehen?
...
→ **Für die Dokumente auf Ebene 1-3 ist ein NIS2 ISMS Starter Kit in Vorbereitung: vorgefertigte, an §30 BSIG und ISO 27001 orientierte Templates mit Ausfüllhilfen. Interesse? [GitHub-Issue öffnen](https://github.com/letaible/nis2scan/issues).**
...
Apache License 2.0, siehe [LICENSE](https://github.com/letaible/nis2scan/blob/main/LICENSE).
...
Repositories und erfordern eine Lizenz. Anfragen: [GitHub Issues](https://github.com/letaible/nis2scan/issues).
